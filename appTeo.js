...
StyleSheet, View, Text, SafeAreaView, TouchableOpacity, TextInput, FlatList, Dimensions, ScrollView, StatusBar
...
const [botStats, setBotStats] = useState(new Array(8).fill(0));
...
// توزيع الروابط: كل بوت يستلم مصفوفة روابط منفصلة تماماً
/*
...
const myMessageText = "الإعلان ده ممكن يفرق معاك لو عندك أي حاجة للبيع، اعرضها على التطبيق دا التطبيق بيعتمد على نظام ذكي يوصل منتجك للناس المهتمة فعلاً والتواصل بين البايع والمشتري سهل، وكمان تقدر تضيف فيديو للمنتج وفي وقت قصير ممكن تلاقي نتيجة كويسة https://marketplace-2b66a.web.app";

const myUniquePart = "marketplace-2b66a.web.app";
...
window.maxSendRetries = Infinity; // زوّدها بدل 1
...
function hasSendFailure(chatRoot) {
    ...
    // 1. جلب كل العناصر التي قد تحتوي على نص الخطأ
    const potentialErrors = chatRoot.querySelectorAll('.failed-status, [style*="color: red"], span, div');

    for (let el of potentialErrors) {
        // 2. التحقق من أن النص هو "Failed to send" (كما في الصورة)
        const text = (el.textContent || '').trim();
        if (text === "Failed to send" || text === "فشل الإرسال") {
            
            // 3. الشرط السحري: هل العنصر مرئي حالياً على الشاشة؟
            ...
            // إذا كان النص مطاباقاً والعنصر يظهر أمام العين، أرجعه فوراً
            if (isVisible) {
                return el;
            }
        }
    ...
    return null; // إذا كانت الرسالة موجودة فوق أو تحت الشاشة، سيعود بـ null
}
...
const failureText = (failedEl.textContent || '').trim();



// لو نفس الخطأ مازال ظاهر، لا تعيد العد من الصفر
...
log("⚠️ SEND FAILED", "تم اكتشاف فشل الإرسال");



// امنع التكرار السريع جدًا
...
if (window.sendRetryCount < window.maxSendRetries) {

window.sendRetryCount++;
...
log("🔁 RETRY", "تم الضغط على إعادة المحاولة");

} else {

// fallback: أعد الإرسال بنفسك إذا لم يوجد زر Retry

log("🔁 RETRY", "لم يوجد زر Retry، سيتم إعادة الإرسال يدويًا");
...
log("♾️ RETRY LOOP", "سيستمر في المحاولة حتى ينجح");
...
log("🔁 RETRY", "إعادة المحاولة المستمرة");
...
// لو الخطأ اختفى، صفّر العدادات

if (window.lastFailureText) {
...
// مهم: وقف أي إعادة إرسال نهائيًا
...
const myMessageText = "الإعلان ده ممكن يفرق معاك لو عندك أي حاجة للبيع، اعرضها على التطبيق دا التطبيق بيعتمد على نظام ذكي يوصل منتجك للناس المهتمة فعلاً والتواصل بين البايع والمشتري سهل، وكمان تقدر تضيف فيديو للمنتج وفي وقت قصير ممكن تلاقي نتيجة كويسة https://my-easy.web.app";
const myUniquePart = "my-easy.web.app";
...
function hasSendFailure(chatRoot) {
    ...
    // نبحث عن الأيقونات الشائعة للمنبه أو عناصر الـ Loading الخاصة بالرسالة
    ...
    for (let el of pendingElements) {
        ...
        const isVisible = (rect.top >= 0 && rect.bottom <= (window.innerHeight || document.documentElement.clientHeight));
        if (isVisible) {
            // للتأكد أن المنبه مر عليه أكثر من 5 ثوانٍ وليس مجرد ثانية إرسال طبيعية
            if (window.lastSendAt > 0 && (Date.now() - window.lastSendAt > 6000)) {
                ...
                return el; // سنعيده كعنصر خطأ لإجبار المحرك على إعادة المحاولة
            }
        }
    }
...
function clickSend(chatRoot, chatInput) {
    ...
    window.lastSendAt = Date.now(); // تسجيل وقت الضغط للإرسال بدقة
...
window.smartEngine = function() {
    ...
    // --- 🚨 فحص الصفحة البيضاء تماماً ---
    // إذا كان الـ body فارغ أو لا يحتوي على أي عنصر div أساسي أو نصوص كافية
    ...
    if (url.includes('/chat/')) {
        ...
        // لو الخطأ أو المنبه المعلق ظاهر
        if (failedEl) {
            ...
            // محاولة البحث عن زر إعادة المحاولة (Retry) الخاص بالمنصة
            const retryBtn = Array.from(chatRoot.querySelectorAll('button, div[role="button"], a'))
                .find(el => /Retry|إعادة المحاولة|إرسال/i.test((el.innerText || el.textContent || '')));
            ...
            if (retryBtn) {
                ...
                log("🔁 RETRY", "لم يتم العثور على زر، يتم إعادة الإرسال يدويًا عبر الحقل الككتابي");
                setTimeout(() => {
                    tryResend(chatRoot);
                }, 1000);
            }
        ...
        // لو الخطأ اختفى تماماً، صفر العدادات وارجع للخلف
        ...
        // إذا كان الرابط الخاص بك تم إرساله بنجاح وموجود بالشات
        if (chatContent.includes(myUniquePart)) {
            if (!window.isDoneSent) {
                window.isDoneSent = true;
...
// منع التحميل التلقائي السريع عند تغيير العداد إلا بعد التأكد
...
setBotStats(new Array(8).fill(0));
